...
class ConvEmbedding(BaseModel):
    def __init__(self, num_filters,
                 ...
                 trainable = True):
        super(ConvEmbedding, self).__init__(name=name, trainable=trainable)

        self.num_filters = num_filters
        self.kernel_size = kernel_size
        self.use_batchnorm = use_batchnorm

        self.emb = Conv2D(filters=self.num_filters,
...
class ConvEmbedding_multistep(BaseModel):
    def __init__(self, num_filters_lst,
                 activate_fn_lst,
                 kernel_size_lst,
                 use_batchnorm_lst,
                 name = 'conv_emb_multistep',
                 regularization = dict(mode='l2', weight=0.0001),
                 trainable = True):
        super(ConvEmbedding_multistep, self).__init__(name=name, trainable=trainable)
